...
let rec length (l : 'a circlist) : int = 
    let Some head = l in (* Save the address of the head cell *)
      let rec counter l' acc = match l with
        |None -> acc
        |Some x -> let (Some y) = (next l') in (* Extract the address of the next head cell *)
                          if y == head then acc +1
                          else counter ( next l') (acc+1) 
...
let to_list (l : 'a circlist)  : 'a list = 
  let rec helper circ size (l1: 'a list) : 'a list = match circ with
      | None -> []
      | Some {p;data;n} -> if (size > 0) then helper (next circ) (size-1) (data :: l1)  else l1    
      in helper l (length l) []
...
let rev (l : 'a circlist) : 'a circlist =
  let Some head  = l in
    let rec order l size = match l with
      |None -> l
      |Some cell -> if size = 0 
                    then if cell == head then next l (*When done reversing list, rotate it one last time*)
                         else l  
                    else begin 
                          let temp = cell.p in
                          cell.p <- cell.n; cell.n <- temp;
                          order ( next l ) (size-1) end
  in order l (length l)
...
let eq (l1 : 'a circlist) (l2 : 'a circlist) : bool = 
  (*il = iteration limiter: if we go through the whole list w/o finding any matches
     il allows us to exit when it equals 0*)
  let initSize = (if length l1 > length l2 then length l1 else length l2) in (*size is the bigger list*)
  let rec compare l1 l2 size (b : bool) =  match l1 with 
    |None -> if length l2 = 0 then true else false
    |Some x -> if size = 0 then b
               else let (Some y) = l2 in
                    if x.data = y.data then compare (next l1) (next l2) (size-1) true
                    else if size != initSize then false
                    else compare l1 (next l2) size false

  in compare l1 l2 initSize false 
